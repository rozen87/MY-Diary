...
export default function ResumeSection({ item, language }: Props) {
  return (
    <section className="relative grid grid-cols-1 gap-8 md:grid-cols-[220px_1fr]">
      ...
      {/* CONTENT */}
      <LayoutBox className="flex flex-col gap-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
        {/* MOBILE TIMELINE */}
        <div className="mb-2 flex items-center gap-3 md:hidden">
          <div className="h-4 w-4 rounded-full border-2 border-white bg-black" />

          <p className="text-xs font-bold tracking-[0.2em] text-black/50">
            {item.year}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {item.highlights[language].map((highlight) => (
            <div key={highlight} className="flex items-start gap-3">
              <div className="mt-3 h-2 w-2 shrink-0 rounded-full bg-black" />
