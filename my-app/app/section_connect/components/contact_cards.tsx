...
export default function ContactCard({ language }: Props) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <>
      <LayoutBox className="flex flex-col items-center justify-center gap-10 py-20 text-center">
        ...
        <div className="flex justify-center pt-6">
          ...
          >
            {language === "ja" ? "お気軽にご連絡ください" : "Let’s Connect"}
          </ConfettiButton>
        </div>
      </LayoutBox>
      ...
      {isOpen && (
        <ModalOverlay onClose={() => setIsOpen(false)}>
          <div className="flex flex-col gap-6 text-center">
            ...
            <p className="text-black/70">wpfha07@yahoo.co.jp</p>

            <a
              href="mailto:wpfha07@yahoo.co.jp"
              className="rounded-full bg-black px-6 py-3 font-semibold text-white transition-all hover:scale-105"
            >
              Send Email
            </a>
          </div>
