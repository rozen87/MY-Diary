...
export default function GalleryItem({
  ...
  return (
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-[32px] border border-white/20 bg-white/10 shadow-2xl backdrop-blur-xl ${sizeClass[size]} `}
    >
      <div className="flex h-full w-full items-center justify-center bg-black/5 p-6">
        <motion.img
          src={src}
          alt={title}
          whileHover={{
      ...
      {/* content */}
      <div className="absolute inset-x-0 bottom-0 flex translate-y-8 flex-col gap-2 p-6 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
        {category && (
