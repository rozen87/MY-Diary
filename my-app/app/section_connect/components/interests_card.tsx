...
export default function InterestsCard({ language, data }: Props) {
  return (
      ...
      <div className="relative z-10 flex flex-col gap-10">
        ...
        <div className="flex flex-wrap gap-4">
          {data.items.map((item) => (
            <div
              key={item}
              className="rounded-full border border-white/20 bg-black/80 px-5 py-3 text-sm font-semibold text-white backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:scale-105"
            >
              {item}
            </div>
          ))}
        ...
        {/* emotional note */}
        <div className="rounded-[28px] border border-white/20 bg-white/30 p-6 backdrop-blur-xl">
